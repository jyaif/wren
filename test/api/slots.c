...
static void noSet(canary_context_t *context)
{
  // Do nothing.
}

static void getSlots(canary_context_t *context)
{
...
static void setSlots(canary_context_t *context)
{
  WrenVM *vm = wrenVMFromContext(context);
...
static void slotTypes(canary_context_t *context)
{
  WrenVM *vm = wrenVMFromContext(context);
  bool result =
...
static void ensure(canary_context_t *context)
{
  WrenVM *vm = wrenVMFromContext(context);
...
static void ensureOutsideForeign(canary_context_t *context)
{
  WrenVM *vm = wrenVMFromContext(context);
...
static void foreignClassAllocate(canary_context_t *context)
{
  WrenVM *vm = wrenVMFromContext(context);
  wrenSetSlotNewForeign(vm, 0, 0, 4);
}

static void getListCount(canary_context_t *context)
{
  WrenVM *vm = wrenVMFromContext(context);
  wrenSetSlotDouble(vm, 0, wrenGetListCount(vm, 1));
}

static void getListElement(canary_context_t *context)
{
  WrenVM *vm = wrenVMFromContext(context);
