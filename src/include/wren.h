...
// Set the number of slots available for the current foreign method to
// [numSlots], growing the stack if needed.
//
...
// Returns the number of elements in the list stored in [listSlot].
...
void wrenPutInMap(WrenVM* vm, int mapSlot, int keySlot, WrenSlot valueSlot);
...
bool wrenRemoveMap(WrenVM* vm, int mapSlot, int keySlot, WrenSlot valueSlot);
  // Clear the map at the given slot
void wrenClearMap(WrenVM* vm, WrenSlot mapSlot);
